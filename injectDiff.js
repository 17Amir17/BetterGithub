// This script run on a differnt site, turns out github 
// uses an iframe that connot be accessed on base page due to cors
// so the script must run there
// Notice the all frames setting in manifest.json

// image-diff.min.js couldn't figure out how to import
// so I copied the min.js to this line do not format xD
...
// The entire login is wrapped in a try and catch to avoid annoying log errors
try {
  ...
  function getImages() {
    // Get before and after images remove cors and sizing
  ...
  function createDiffView(image1, image2) {
    // Create view element and combined image with imagediff
    ...
    // Resize the canvas by default dimensions
    const resizeBy =
  ...
  function waitForImageData(){
    // Wait for the images to have data because github for some reason
    // Gives images a src after the page has loaded
  ...
  (async () => {
    ...
    function onImageLoad() {
      // When both images load add the view
      iCount++;
      if (iCount != 2) return;
